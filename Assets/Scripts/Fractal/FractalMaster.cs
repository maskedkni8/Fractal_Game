...
public class FractalMaster : MonoBehaviour {
    ...
    private float powerIncreaseSpeed = 0.08f;
    private float darknessSpeed = 50f;
    ...
    void Update () {
        if (Application.isPlaying) {
            fractalPower += powerIncreaseSpeed * Time.fixedDeltaTime;
            if (fractalPower > 10)
            {
                fractalPower = 0.1f;
            }

            darkness += darknessSpeed * Time.smoothDeltaTime;
            if (darkness > 100)
            {
                darkness = 1;
            }
